 ...
 */
public class SummaryElement extends XmlElement {

  /**
   * Creates a new hAtom "entry-title" element for the given entry.
   */
  public SummaryElement(BaseContentEntry<?> entry) {
    this(entry, "span");
  }
   ...
   * entry.
   */
  public SummaryElement(BaseContentEntry<?> entry, String elementType) {
    super(elementType);
    checkNotNull(entry, "entry");
    checkNotNull(elementType, "elementType");
    this.setAttribute("class", "entry-summary");
    this.addText(entry.getSummary().getPlainText());
  }  
}
